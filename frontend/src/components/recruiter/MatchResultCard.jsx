...
export default function MatchResultCard({ match, onToggleShortlist, pending = false }) {
  ...
  return <div>
    ...
    <section className="detail-section"><h3>What supports this match</h3>
      {match.reasons?.length > 0 && <ul className="mb-3 list-disc space-y-2 pl-5 text-sm text-muted-foreground">{match.reasons.map((reason, index) => <li key={index}>{reason}</li>)}</ul>}
      {match.explanation && <p className="text-muted-foreground">{match.explanation}</p>}
      {!match.explanation && !match.reasons?.length && <p className="text-muted-foreground">No explanation is available for this result.</p>}
